...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for class_idx in range(1000):
        ...
        split = 100 # the number of samples chosen to be validation set

        val_subset.append(class_sample_index[:split]) # validation samples
        train_subset.append(class_sample_index[split:]) # training samples
    ...
    print('number of train set: ', len(flattened_train_subset))
    print('number of val set: ', len(flattened_val_subset))
    print('sum of train set and val set ', len(flattened_val_subset) + len(flattened_train_subset))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc1, acc5, test_loss = validate(valid_from_train_loader, model, criterion, args)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        ## resnet 50 only ### generate a random mask
        num_bottlenecks = [3, 4, 6, 3]
        random_int = np.random.randint(0, 2, sum(num_bottlenecks))
        random_mask = ''
        for value in random_int:
            random_mask = random_mask + str(value)
        cumsum = np.cumsum(num_bottlenecks)
        code1 = int(random_mask[0:cumsum[0]], 2)
        code2 = int(random_mask[cumsum[0]:cumsum[1]], 2)
        code3 = int(random_mask[cumsum[1]:cumsum[2]], 2)
        code4 = int(random_mask[cumsum[2]:cumsum[3]], 2)
        config = (code1, code2, code3, code4) # converted to decimal number
        ...
        if i % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Acc@5 {top5.val:.3f} ({top5.avg:.3f})--{config}'.format(
                   epoch, i, len(train_loader), batch_time=batch_time,
                   data_time=data_time, loss=losses, top1=top1, top5=top5, config = config))
