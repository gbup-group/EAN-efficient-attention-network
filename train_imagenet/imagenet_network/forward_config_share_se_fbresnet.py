...
__all__ = ['forward_config_share_se_resnet50']
...
class Attention(nn.Module):
    def __init__(self, ModuleList, block_idx):
        ...
        if block_idx == 1:
            self.share_fc = SEModule(64*4, 16)
        elif block_idx == 2:
            self.share_fc = SEModule(128*4, 16)
        elif block_idx == 3:
            self.share_fc = SEModule(256*4, 16)
        elif block_idx == 4:
            self.share_fc = SEModule(512*4, 16)
...
def forward_config_share_se_resnet50(num_classes=1000, pretrained = False):
...
if __name__ == '__main__':
    # model = config_share_se_resnet50(1000, False, (1,2,3,4)).cuda()
    model = forward_config_share_se_resnet50(1000, False).cuda()
